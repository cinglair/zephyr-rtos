...
static int cmd_signal_generate(const struct shell *sh, size_t argc, char **argv, void *data)
{
	...
	if (signal == 1) {
		...
		HAL_DAC_Start_DMA(&hdac, DAC1_CHANNEL_1, (uint32_t *)sin_wave, 256,
				  DAC_ALIGN_12B_R);
...
SHELL_SUBCMD_DICT_SET_CREATE(sub_signal_generate, cmd_signal_generate, (sine, 1, "SENO"),
...
static int print_cmd_handler(const struct shell *sh, size_t argc, char **argv, void *data)
{
	...
	if ((first_harm >= 0) && (number_harm > 0)) {
		shell_print(sh, "Arguments FFT (%d, %d): ", first_harm, number_harm);
